...
class TopAppartmentsPanel extends JPanel {
    ...
    private void fetchData(String selectedCity) {
        ...
        query += " GROUP BY a.appartement_id, a.nom ORDER BY nb_locations DESC, a.nom ASC FETCH FIRST 5 ROWS ONLY";
...
private void fetchData() {
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        data.clear(); // Assurez-vous de vider les données précédentes
        while (rs.next()) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.setFont(new Font("Segoe UI", Font.BOLD, 18));
        g2.setColor(textColor);
        g2.drawString("Repartition des Biens par Type d'Appartement", width / 2 - 200, 40);
