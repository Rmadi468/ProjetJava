...
public class MesLocations extends JFrame {
    ...
    try (Connection conn = DBConnection.getConnection()) {
        // Étape 1 : Récupérer la date de début de la location
        String query = "SELECT date_debut FROM locations WHERE location_id = ?";
        ...
        if (rs.next()) {
            ...
            Date dateActuelle = new Date(); // Date système actuelle

            // Étape 2 : Comparer les dates
            if (dateDebut.after(dateActuelle)) {
                // Étape 3 : Confirmer l'annulation
                ...
                JOptionPane.showMessageDialog(this,
                        "Vous ne pouvez pas annuler cette location car elle a déjà commencé ou est terminée.",
                        "Annulation impossible", JOptionPane.WARNING_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this,
                    "Aucune location trouvée avec l'ID spécifié.",
                    "Erreur", JOptionPane.ERROR_MESSAGE);
        }
    } catch (SQLException e) {
