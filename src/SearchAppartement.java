...
public class SearchAppartement extends JFrame {
    ...
    public SearchAppartement(int clientId) {
        ...
        filterTypeBox = new JComboBox<>(new String[]{"Tous", "studio","villa", "duplex"});
    ...
    private void loadAppartements() {
        model.setRowCount(0);
        try (Connection conn = DBConnection.getConnection()) {
            String query = "SELECT * FROM appartements WHERE disponibilite = 1 ";
            PreparedStatement stmt = conn.prepareStatement(query);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                model.addRow(new Object[]{
    ...
    private void faireReservation(int appartementId) {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            try (Connection conn = DBConnection.getConnection()) {
                ...
                String checkQuery = "SELECT COUNT(*) FROM locations WHERE appartement_id = ? AND ((date_debut <= TO_DATE(?, 'YYYY-MM-DD') AND date_fin >= TO_DATE(?, 'YYYY-MM-DD')) OR (date_debut <= TO_DATE(?, 'YYYY-MM-DD') AND date_fin >= TO_DATE(?, 'YYYY-MM-DD')) OR (date_debut >= TO_DATE(?, 'YYYY-MM-DD') AND date_fin <= TO_DATE(?, 'YYYY-MM-DD')))";
                ...
                String insert = "INSERT INTO locations (client_id, appartement_id, date_debut, date_fin, nombre_personnes) VALUES (?, ?, TO_DATE(?, 'YYYY-MM-DD'), TO_DATE(?, 'YYYY-MM-DD'), ?)";
                ...
                if (prixRs.next()) {
                    ...
                    if (clientFidele) {
                        ...
                        JOptionPane.showMessageDialog(this,
                            ...
                            "Vous paierez : " + String.format("%.2f", totalAvecRemise) + "dt au lieu de " + String.format("%.2f", total) + "dt.",
                            "Reservation confirmee", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(this,
                            "Votre reservation a ete confirmee.\nMontant total : " + String.format("%.2f", total) + "dt.",
                            "Reservation confirmee", JOptionPane.INFORMATION_MESSAGE);
                    }
                }
